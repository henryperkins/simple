    ...
    async def process_file(self, file_path: Path, output_base: Path) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            try:
                ...
                module_name = file_path.stem or file_path.name.split('.')[0]  # Fallback for module name
                ...
                # Ensure module_name is set in metadata
                if 'module_name' not in context.metadata:
                    context.metadata['module_name'] = module_name

                self.logger.debug(f"Initialized context with module_name: {context.metadata.get('module_name')}")
